...
Каталог `bot_old` содержит прежнюю логику Telegram-бота и мини‑приложение. В `bot` лежит минимальный `package.json` для линтера, полноценный код остаётся в `bot_old`.
