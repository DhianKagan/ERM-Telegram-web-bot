...
- Руководство по настройке Telegram-бота включено в этот файл, `docs/telegram_bot_manual.md` удалён
...
- Корневой `package.json` хранит зависимости `eslint` и `jiti`, линтер запускается командой `npx eslint bot/src`
...
- Уточнены типы Express и логов, сборка Docker проходит
...
- Скрипт `install_bot_deps.sh` устанавливает корневые, серверные и клиентские зависимости и завершает установку,
...
- Все технические файлы сведены в `docs/technical_manual.md`
- Руководство по настройке Telegram-бота включено туда, `docs/telegram_bot_manual.md` удалён
...
- Сервисы `LogsService` и `UsersService` используют интерфейсы репозитория, контроллеры типизированы Express
