...
test('logout очищает новые и legacy token cookies', async () => {
  ...
  const tokenCalls = res.clearCookie.mock.calls.filter(
    ([name]) => name === 'token',
  );

  expect(tokenCalls).toHaveLength(2);
  expect(tokenCalls[0]).toEqual([
    'token',
    expect.objectContaining({
      path: '/',
      secure: true,
