...
- Загрузка в Mapbox выполняется командой `mapbox upload <пользователь>.<tileset> dist/addresses.mbtiles`. CLI ставится через `python3 -m pip install --user mapboxcli`, токен авторизации передаётся переменной `MAPBOX_ACCESS_TOKEN`.
- Workflow `.github/workflows/refresh-addresses.yml` запускается по понедельникам в 01:30 UTC (04:30 по Киеву, UTC+3), использует секрет `MAPBOX_ACCESS_TOKEN` и repository variables `MAPBOX_USERNAME`, `MAPBOX_TILESET_ID`, публикуя слой в Tilesets и прикрепляя артефакт сборки.
