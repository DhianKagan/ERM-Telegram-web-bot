...
router.get('/:id', (0, auth_1.default)(), (0, express_validator_1.param)('id').isMongoId(), async (req, res, next) => {
    try {
        ...
        const safeName = path_1.default.basename(file.name ?? file.path ?? 'file');
...
router.delete('/:id', (0, auth_1.default)(), (0, express_validator_1.param)('id').isMongoId(), async (req, res, next) => {
    try {
        ...
        await (0, dataStorage_1.deleteFile)(req.params.id);
    ...
    try {
        ...
        const uploadedAtSource = file.uploadedAt instanceof Date
            ...
            : typeof file.uploadedAt === 'string'
                ...
                : now;
        const uploadedAt = uploadedAtSource instanceof Date && !Number.isNaN(uploadedAtSource.getTime())
            ? uploadedAtSource
            : now;
        ...
        const uploadedBy = typeof uploadedByRaw === 'number' && Number.isFinite(uploadedByRaw)
            ? uploadedByRaw
            : 0;
        const thumbnailUrl = typeof file.thumbnailPath === 'string' && file.thumbnailPath.trim().length > 0
        ...
        const nextAttachments = existingAttachments.filter((attachment) => (0, attachments_1.extractFileIdFromUrl)(attachment?.url) !== fileId);
