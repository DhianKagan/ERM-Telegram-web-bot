...
- Страница `/admin` построена на AdminJS и заменяет прежние формы
- Модуль `@adminjs/express` подключается динамически через `import()`
- Модуль `@adminjs/mongoose` также загружается динамически
- AdminJS импортируется через `{ default }`, иначе `registerAdapter` не работает
- AdminJS инициализируется после успешного подключения к MongoDB
