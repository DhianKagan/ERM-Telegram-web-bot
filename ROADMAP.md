...
66. Leaflet подключён локально, политика CSP разрешает тайлы OpenStreetMap
67. Политика CSP разрешает подключение к сервису ORSM
68. Фильтры маршрутов отправляются только при заполнении
69. Страница `/tasks` автоматически обновляет список после создания задачи,
    добавлена кнопка «Обновить задачи»
70. Страница маршрутов отображает путь через OSRM,
    выбор диапазона дат реализован календарём
