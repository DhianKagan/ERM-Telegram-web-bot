...
Сервисы регистрируются через библиотеку `tsyringe` в файле `bot/src/di/index.ts`.
...
Планировщик `KEY_ROTATION_CRON` пересоздаёт ключи, разрешённые
источники для R2 задаются `R2_CORS_ORIGIN`.
...
Таблицы реализованы на AG Grid. Сервис `useGrid` задаёт общие фильтры и пагинацию, а конфигурации колонок вынесены в модули `bot/web/src/columns/*` для повторного использования.

## DSL форм

Схемы форм описываются в `bot/src/form/taskForm.schema.json`.
Структура:
