...
- CI ограничен веткой `main`, pull request снова автоматически запускают тесты и линтеры без дополнительных веток.
- Workflow `Release` запускается при push в `main` и при тегах `v*.*.*`, что возвращает автоматический деплой на Railway.
- Перед деплоем workflow `Release` прогоняет Chromium e2e‑тесты Playwright и публикует диагностику в summary.
- Workflow `Docker` подставляет тестовые значения секретов, если GitHub Actions не выдаёт реальные (например, в PR из форков), поэтому сборка и тесты проходят до запуска деплоя на Railway.
- README и CONTRIBUTING обновлены: политика линейной истории `main`, описание запуска проверок и деплоя.
- Workflow `Release` разворачивает проект через `railway up --yes`; добавлен шаблон `railway.json` и документация по секретам CI/CD.
- Railway возвращён к Docker-сборке: `railway.json` ссылается на `Dockerfile`, образ устанавливает `curl` и синхронизирует команды бота при старте.

- Привязка вложений задач учитывает права пользователя и устанавливает taskId у всех переданных файлов.
- Раздел «Маршруты» переименован в «Логистика»; обновлены ссылки `/cp/logistics`, `/mg/logistics`, хлебные крошки и сообщение бота о расстоянии.
- Добавлен сигнатурный антивирус (`ANTIVIRUS_VENDOR=signature`) с конфигурируемыми сигнатурами и fallback на ClamAV.
- Удаление задач удаляет сообщения бота в чатах и личных сообщениях, переносит данные в архив с пометкой времени и автора; добавлены API `/api/v1/archives` и раздел «Архив» с полным удалением для маски 8.
- Антивирус для вложений переключён на clamd через `clamdjs`, добавлены настройки окружения и unit-тесты.
...
- Добавлен уровень `ACCESS_TASK_DELETE = 8`, удаление задач требует этой маски; логика авторизации и интерфейс обновлены.
...
- Size-limit проверяет `index-*.js` в `apps/api/public/js`, что предотвращает ложные срабатывания.
...
- Модель AuthUser и сервис авторизации поддерживают роль «manager».
- Настроена интеграция Lighthouse CI через GitHub App с временным публичным хранилищем.
- Middleware проверки JWT перенаправляет на `/login` при отсутствии авторизации.
- Маршрут сотрудников заполняет отдел, дивизион и должность и поддерживает проекцию полей.
- Переименованы вкладки «Сотрудники», «Департаменты», «Флоты» и «Роли»;
  добавлены вкладки «Департамент», «Отдел», «Должность», «Сотрудник» и «Автопарк».
  Формы теперь подтверждают сохранение и удаление через модалки.
- На странице «Настройки» добавлены вкладки сотрудников, департаментов, флотов и ролей с поиском и пагинацией; реализованы сервисы `/api/v1/collections` и модалки подтверждения.
- Добавлена модель CollectionItem с индексами и репозиторием поиска с пагинацией.
- Переименована страница «Роли» в «Настройки», удалён верхний поиск,
  управление коллекциями перенесено на эту страницу; переменная
  `VITE_COLLECTIONS_READY` больше не используется. Название проекта
  изменено на ERM.
- В боковое меню добавлен пункт «Коллекции», активируемый `VITE_COLLECTIONS_READY`.
- Добавлены CRUD роуты для флотов, департаментов и сотрудников.
- Добавлены CRUD роуты для коллекций, мутирующие защищены `requireRole('admin')`.
...
- История задач фиксирует автора изменений и значения до/после.
...
- Dockerfile запускает `ensureDefaults` при старте контейнера, токен бота задаётся переменной окружения `BOT_TOKEN`.
