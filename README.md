...
## Интерфейс
Мини‑приложение основано на Vite и Tailwind. Дизайн полностью взят из TailAdmin: Sidebar и Header используют иконки `@heroicons/react`. Стартовая страница `/dashboard` показывает метрики в виде карточек и будет подключена к API.
...
### Итог
Проект представляет готовую базу и быстро разворачивается, но требует модернизации для полноценного продакшна. Интерфейс построен на React + Vite + Tailwind и использует дизайн TailAdmin. Код фронтенда форматируется Prettier.
