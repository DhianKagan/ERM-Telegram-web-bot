...
const getMatrixBuilder = () => currentMatrixBuilder ?? ((points, options) => (0, graphhopperAdapter_1.buildTravelMatrix)(points, options));
const getOrToolsSolver = () => currentOrToolsSolver ?? orToolsAdapter_1.solveWithOrTools;
...
const buildSolverPayload = (depot, tasks, distanceMatrix, options) => {
    ...
    return {
        ...
        time_limit_seconds: typeof options.timeLimitSeconds === 'number' && options.timeLimitSeconds > 0
...
const optimizeLegacy = async (taskIds, count = 1, method = 'angle', actorId) => {
    ...
    return (0, routePlans_1.createDraftFromInputs)(routeInputs, { actorId, method, count }, hints);
};
