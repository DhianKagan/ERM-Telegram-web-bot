...
async function syncVehicleAssignments(previous, current) {
    ...
    const prevVehicleId = prevRequires && previous?.transport_vehicle_id
        ? String(previous.transport_vehicle_id)
        : null;
    const nextVehicleId = nextRequires && current?.transport_vehicle_id
...
const mergeAttachmentSources = (current, sources) => {
    ...
    return {
        ...sources.previous,
        ...current,
        name: pickString(current.name, sources.previous?.name, sources.file?.name) ??
            current.name,
        url: sources.file ? (0, fileUrls_1.buildFileUrl)(sources.file._id) : current.url,
        thumbnailUrl: pickString(current.thumbnailUrl, sources.previous?.thumbnailUrl) ??
            thumbnailFromFile,
        uploadedBy: pickNumber(current.uploadedBy, sources.previous?.uploadedBy, sources.file?.userId) ??
            current.uploadedBy,
        uploadedAt,
        type: pickString(current.type, sources.previous?.type, sources.file?.type) ??
            (current.type ?? 'application/octet-stream'),
        size: pickNumber(current.size, sources.previous?.size, sources.file?.size) ??
            current.size,
    };
};
async function enrichAttachmentsFromContent(data, previous) {
    ...
    if (attachmentsRaw === undefined) {
        ...
        const derived = (0, attachments_1.buildAttachmentsFromCommentHtml)(commentHtml ?? '', {
    ...
    const attachments = attachmentsRaw.map((item) => ({ ...item }));
...
async function updateTask(id, fields, userId) {
    ...
    const assignedUserId = typeof prev.assigned_user_id === 'number' ? prev.assigned_user_id : undefined;
...
async function updateTaskStatus(id, status, userId, options = {}) {
    ...
    if (status === 'Выполнена' && currentStatus) {
        const allowedCompletionSources = [
            'Новая',
            'В работе',
            'Отменена',
            'Выполнена',
        ...
        if (!allowedCompletionSources.includes(currentStatus)) {
            const err = new Error('Статус «Выполнена» доступен только после этапа «В работе»');
            err.code = 'TASK_STATUS_INVALID';
            throw err;
        }
    }
...
async function findNextTelegramId() {
    const last = await model_1.User.findOne({}, { telegram_id: 1 })
        ...
        .lean()
        .exec();
    let candidate = typeof last?.telegram_id === 'number' && Number.isFinite(last.telegram_id)
        ? last.telegram_id + 1
        : 1;
...
async function updateUser(id, data) {
    ...
    if (typeof sanitized.access === 'number') {
        const previousAccess = typeof previous?.access === 'number' ? previous.access : null;
