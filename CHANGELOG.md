...
- Импорт `@adminjs/express` переведён на динамический, так как пакет доступен только как ES‑модуль
- Импорт `@adminjs/mongoose` также загружается через `import()` для совместимости
