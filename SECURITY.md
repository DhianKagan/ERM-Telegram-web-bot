...
| Secrets hygiene        | Author PR + Maintainer                  | Каждый PR/release, особенно при изменении CI, deploy, `.env.example`, интеграций и webhook/SDK                                 | Проверка diff на отсутствие секретов, подтверждение что секреты хранятся только в GitHub/Railway secrets; при ротации — запись в incident ticket/postmortem | Частично автоматизировано: `pnpm run docs:audit` проверяет docs security hygiene; отдельного repo-wide secret-scanning workflow для всего дерева пока нет      |
