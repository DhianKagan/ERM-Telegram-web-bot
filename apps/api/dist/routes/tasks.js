...
const readTaskIdFromRequest = (req) => {
    var _a, _b;
    const body = req.body;
    const bodyTaskId = selectTaskIdCandidate(body === null || body === void 0 ? void 0 : body.taskId);
    if (bodyTaskId)
        return bodyTaskId;
    const bodySnakeTaskId = selectTaskIdCandidate(body === null || body === void 0 ? void 0 : body.task_id);
    if (bodySnakeTaskId)
        return bodySnakeTaskId;
    const queryTaskId = selectTaskIdCandidate((_a = req.query) === null || _a === void 0 ? void 0 : _a.taskId);
    if (queryTaskId)
        return queryTaskId;
    const querySnakeTaskId = selectTaskIdCandidate((_b = req.query) === null || _b === void 0 ? void 0 : _b.task_id);
    if (querySnakeTaskId)
...
const syncAttachmentsForRequest = async (req, attachments) => {
    ...
    const userId = resolveNumericUserId((_a = req.user) === null || _a === void 0 ? void 0 : _a.id);
        ...
        if (files.length > 0) {
            ...
            if (shouldApplyGrace && cutoff) {
                try {
                    ...
                    const staleEntries = await (typeof staleQuery
                        .exec === 'function'
                        ? staleQuery.exec()
            ...
            const stats = {
                count: (_b = rawStats.count) !== null && _b !== void 0 ? _b : 0,
                size: (_c = rawStats.size) !== null && _c !== void 0 ? _c : 0,
            };
...
const handleInlineUpload = async (req, res) => {
    ...
    try {
        ...
        const finalizeResult = await (0, uploadFinalizer_1.finalizePendingUploads)({
            req: req,
            attachments: (_a = bodyWithAttachments.attachments) !== null && _a !== void 0 ? _a : [],
        });
        bodyWithAttachments.attachments =
            finalizeResult.attachments;
        if (res.headersSent)
            return;
        const attachment = (_b = bodyWithAttachments.attachments) === null || _b === void 0 ? void 0 : _b[0];
        ...
        if (idx + 1 === total) {
            ...
            try {
                finalizeResult = await (0, uploadFinalizer_1.finalizePendingUploads)({
                    req: req,
                    attachments: (_b = bodyWithAttachments.attachments) !== null && _b !== void 0 ? _b : [],
                });
            }
            finally {
                cleanupAll();
            ...
            if (res.headersSent)
                return;
            const attachment = (_c = bodyWithAttachments.attachments) === null || _c === void 0 ? void 0 : _c[0];
            if (!attachment) {
                res.sendStatus(500);
                return;
            }
...
const normalizeArrays = (req, _res, next) => {
    ...
    const assignedRaw = (_b = body.assigned_user_id) !== null && _b !== void 0 ? _b : body.assignedUserId;
