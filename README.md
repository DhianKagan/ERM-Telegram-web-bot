...
Планировщик `scheduler.js` запускается вместе с ботом и раз в минуту проверяет задачи с полем `remind_at`. Когда срок наступает, бот отправляет сообщение в чат и очищает поле. Периодичность задаётся переменной `SCHEDULE_CRON`.
