...
- При деплое на Railway Procfile выполняет `scripts/set_bot_commands.sh`,
 ...
 - Форма задачи описана в модуле `bot/shared/taskFields.cjs` и используется
   как в веб‑приложении, так и в чат‑боте. Модуль содержит полный набор полей
   для создания и редактирования задач и подходит для мини‑приложения Telegram.
- В `TaskForm` скорректирован относительный путь к модулю `taskFields.cjs`.
