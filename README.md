...
- Кнопка создания задачи теперь называется «Новая задача», форма позволяет выбрать стартовую и финальную точку на карте, указать тип задачи и упомянуть исполнителей через `@id` Telegram.
...
- При повторной регистрации больше не возникает ошибка MongoDB о дублирующемся индексе.
