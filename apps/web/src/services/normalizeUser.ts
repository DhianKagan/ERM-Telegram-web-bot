...
export const getReferenceId = (value: ReferenceLike): string | undefined => {
  ...
  return toTrimmedString(withId._id) || toTrimmedString(withId.id);
};
...
export const normalizeUser = <T extends Partial<User>>(
  user: T,
): T & NormalizedFields => {
...
export const normalizeUsers = <T extends Partial<User>>(
  users: T[],
): Array<T & NormalizedFields> => users.map((item) => normalizeUser(item));
