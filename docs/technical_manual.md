...
Полный список маршрутов можно собрать скриптом `pnpm --filter ./apps/api build:swagger`; статический HTML и `openapi.json` окажутся в каталоге `docs/api`. Ниже перечислены базовые эндпойнты:
