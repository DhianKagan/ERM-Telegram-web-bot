...
- Для произвольных методов можно вызвать `call('method', params)` из `telegramApi.ts`.
...
сохраняет его в `localStorage` и передаёт в заголовке `X-XSRF-TOKEN`.
При ошибке 403 запрос токена можно повторить.
