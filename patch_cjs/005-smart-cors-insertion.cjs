...
// 1) уберём все старые неудачные вставки
...
const m = s.match(re);
if (!m) {
  console.error('❌ В server.ts не найдено присваивание express(...). Покажи первые 40 строк файла — подстрою патч.');
  process.exit(1);
}
const varName = m[2];

// 3) вставим X.use(corsMiddleware) сразу после объявления
