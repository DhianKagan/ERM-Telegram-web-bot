...
Для Lighthouse CI получите build‑token командой `npx lhci wizard` и сохраните его в секрете GitHub Actions `LHCI_TOKEN`; для локальной отладки добавьте переменную в `.env`.
