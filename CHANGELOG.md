...
- Каталог `bot` снова включает код бота и веб‑клиента; папка `bot_old` удалена как неактуальная.
- Основной образ собирается через `bot/Dockerfile`; корневой `Dockerfile` служит для единой сборки без Compose.
- В `.dockerignore` добавлены `.env` и `.env.*` для исключения переменных из контекста Docker.
- Настроен базовый workflow `ci.yml`: зависимости ставятся через `npm ci --prefix bot`, затем запускаются тесты.
- Планировщик запускается только вне режима тестов, остановка доступна через `stopScheduler()`.
- Добавлена поддержка GatewayAPI: коды подтверждения отправляются через SMS или Telegram.
...
- В этом workflow добавлен шаг, который выводит `MONGO_DATABASE_URL` без пароля и прерывает сборку при неверном формате.
- Далее команда `grep -E '^MONGO_DATABASE_URL|^APP_URL' .env` подтверждает корректное применение переменных.
- По завершении workflow файл `.env` удаляется, чтобы исключить утечку секретов.
...
- Скрипт также ищет `mongoose` в `bot/node_modules` на случай отсутствия глобальной зависимости.
- Перед подключением скрипт выводит домен и имя базы без логина и пароля, чтобы быстрее находить ошибки в URL.
- При ошибке "bad auth" скрипт подсказывает проверить логин и пароль в `MONGO_DATABASE_URL` и предупреждает, если строка подключения не содержит учётных данных.
- Финальный слой Dockerfile задаёт переменную `NODE_ENV=production`, что сокращает объём образа.
- Сборочный слой теперь использует директорию `/app/bot`, за счёт чего логи при билде короче.
...
- Мини-приложение использует только светлую тему, классы `dark` удалены.
- Из конфигурации Tailwind убран `darkMode`, в README и компонентах удалены упоминания тёмной темы.
- Добавлена команда `/upload_voice <taskId>` для загрузки голосовых сообщений в R2.
- В `TelegramLogin` появилось предупреждение при отсутствии `VITE_BOT_USERNAME`.
- В README добавлена подсказка о проверке домена `APP_URL` в BotFather, чтобы устранить ошибку `Username invalid`.
- Удалены `package.json` и `package-lock.json` из корня,
  больше не используемые модулем кристаллизации.
...
- При регистрации пользователя вызывается `verifyUser`, результат сохраняется в `verified_at`.
- При ошибке `BOT_VERIFIER_FORBIDDEN` бот выводит пояснение и завершает работу.
- При ошибке подключения к MongoDB бот делает три повторные попытки.
  В README дополнено разъяснение: при 502 проверьте DNS кластера и `MONGO_DATABASE_URL`.
- Введена переменная `BOT_PORT` для вебхука Telegraf, что позволяет запускать API и бот на разных портах без ошибок `EADDRINUSE`.
- README дополнен шагом подключения плагина MongoDB на Railway.
- В README появился подраздел об ошибке `bad auth: Authentication failed`.
- В README после установки зависимостей добавлен шаг
  `node scripts/check_mongo.cjs` для проверки подключения к базе.
...
- Файл `.env` удалён из репозитория, README содержит напоминание создать его из `.env.example`.
...
- Для отправки сообщений планировщик использует очередь `p-queue` (до 30 запросов в секунду).
...
- Скрипт выводит домен и имя базы перед подключением, не раскрывая конфиденциальные данные.
...
- Дополнительно в `bot/web/package.json` заданы override для `rimraf@6` и `uuid@9`, что убрало сообщения об их устаревших версиях.
...
- Удалена пустая директива `@apply;` в `bot/web/src/index.css`, из-за которой сборка Tailwind завершалась ошибкой.
...
- Добавлен маршрут `/`, выдающий `index.html`; теперь запрос к корню сайта не возвращает 403.
...
- Удалены устаревшие комментарии `/* global fetch */`, теперь ESLint не выдаёт ошибку `no-redeclare`.
...
- Маршруты `/auth/register` и `/api/auth/telegram` валидируют входные данные через `express-validator`.
...
- Добавлен скрипт `scripts/setup_tdweb.sh` для установки TDWeb и подготовки файлов WebAssembly.
- Страница `/chats` снова активна и загружает TDLib из каталога `bot/web/public/tdlib`.
- Страница `/chats` использует TDWeb без сторонних библиотек.
...
- Перед запуском линтера фронтенда необходимо установить зависимости командой
...
- Вход администратора выполняется через Telegram ID, переменные ADMIN_EMAIL и ADMIN_PASSWORD удалены.
- Переменные `VITE_TELEGRAM_API_ID` и `VITE_TELEGRAM_API_HASH` добавлены в пример и отмечены как необязательные.
- Vite собирает фронтенд прямо в `bot/public`, копирование из `web/dist` больше не используется.
- Исправлена ошибка `Переменная BOT_TOKEN не задана` при запуске тестов:
  переменные окружения задаются в начале каждого тестового файла.
- В `api.js` проверяется существование `bot/public/index.html` через
  `fs.existsSync` и `fs.statSync`, что точнее определяет необходимость сборки
  фронтенда.
