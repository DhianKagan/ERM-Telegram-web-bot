...
const finalizePendingUploads = async (options) => {
    ...
    if (Array.isArray(options.attachments)) {
        ...
        const current = await model_1.Task.findById(normalizedTaskId)
            .select(['attachments'])
            .lean();
        if ((current === null || current === void 0 ? void 0 : current.attachments) && Array.isArray(current.attachments)) {
    ...
    try {
        for (const entry of pending) {
            ...
            await (0, moveFile_1.moveFile)(entry.tempPath, finalPath);
            ...
            if (entry.tempThumbnailPath) {
                ...
                try {
                    await (0, moveFile_1.moveFile)(entry.tempThumbnailPath, thumbTarget);
