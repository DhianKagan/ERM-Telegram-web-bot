...
16. Импорт AdminJS осуществляется через `{ default }` для корректной работы `registerAdapter`
17. AdminJS инициализируется после успешного подключения к MongoDB
...
20. Расширенная модель задачи с логистикой, закупками и работами
21. Поддержка enum-полей для типов транспорта, оплаты и статусов
