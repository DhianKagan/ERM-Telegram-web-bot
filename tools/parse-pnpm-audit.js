...
function handleAdvisories(obj) {
  ...
  items.forEach((a) => {
    ...
    const paths =
      (a.findings && a.findings.map((f) => f.paths).flat()) || a.paths || [];
    console.log(
      `${severity.padEnd(9)} | ${pkg.padEnd(30)} | vuln: ${vulnerable.padEnd(18)} | patched: ${patched.padEnd(12)}`,
    );
    if (paths.length) {
    ...
    if (patched && patched !== 'none' && patched !== '0') {
      console.log(
        `  suggested: pnpm -w add ${pkg}@^${patched.replace(/[^0-9.\-^]/g, '')}`,
      );
    } else {
      console.log(
        `  suggested: проверить upstream (не найдено patched_versions)`,
      );
    }
    printLine();
  });
...
if (raw.vulnerabilities && Object.keys(raw.vulnerabilities).length) {
  ...
  entries.forEach(([pkg, info]) => {
    ...
    const patched =
      info.patched_versions || info.fixAvailable?.version || 'unknown';
    console.log(
      `${severity.padEnd(9)} | ${pkg.padEnd(30)} | patched: ${String(patched).padEnd(12)}`,
    );
    ...
    if (patched && patched !== 'unknown') {
      console.log(
        `  suggested: pnpm -w add ${pkg}@^${String(patched).replace(/[^0-9.\-^]/g, '')}`,
      );
    } else {
      console.log(
        '  suggested: проверить конкретную транзитивную зависимость или обновить родительский пакет',
      );
    }
    printLine();
  });
...
console.log(
  'Не удалось распознать стандартную структуру audit JSON. Доступные ключи в файле:',
  Object.keys(raw),
);
