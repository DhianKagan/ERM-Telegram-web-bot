...
Если в браузере возникает сообщение `Minified React error #130`, убедитесь, что сервер отдаёт HTML с элементом `<div id="root">`. В `main.tsx` теперь предусмотрена проверка и понятное сообщение об отсутствии этого элемента.
