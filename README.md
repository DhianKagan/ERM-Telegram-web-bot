...
Проект проверен на Node.js 20 и npm 11. Убедитесь, что в продакшене используются
эти версии или совместимые с ними.

## Возможности

- Управление задачами через чат и мини‑приложение
- REST API доступно на префиксе `/api/v1`, описание на `/api-docs`
- В ответах API поле `username` содержит Telegram ID пользователя
- Дополнительно возвращается `telegram_username` с оригинальным username
- В интерфейсе и сообщениях ID отображается ссылкой с именем пользователя
- Перед обращением к базе Telegram ID проверяется как число
- Имена в полях `controllers`, `assignees` и `created_by` отображаются ссылками на профиль
- При открытии задачи имена новых участников подгружаются автоматически
- Веб-панель администратора находится на пути `/cp` и доступна только админам.
  Токен хранится в cookie и передаётся автоматически
- Запросы защищены от CSRF через middleware `lusca`
- CSRF-токен кладётся в cookie `XSRF-TOKEN` и автоматически передается функцией `authFetch`
- Маршруты `/api/v1/auth/send_code` и `/api/v1/auth/verify_code` не требуют CSRF-токена
- Сессии хранятся в MongoDB через `connect-mongo`
- Страница `/cp/roles` позволяет настраивать права ролей
- Для быстрого сравнения прав введена маска доступа: 1 - пользователь, 2 - администратор
- Администратора можно назначить, установив `access: 2` в коллекции `telegram_users`
- Маршрут `/cp/admin` использует `/*splat` для Express 5
- Комментарии в `api.js` уточняют использование `/*splat` вместо устаревшего
  `/{*splat}`
- В шапке вместо меню профиля отображается кнопка «Админка»
- Напоминания и уведомления
- Команда `/register` проверяет и регистрирует пользователя
- Авторизация выполняется только кодом, который бот отправляет в Telegram
- Код входа для администратора или пользователя определяется автоматически
- JWT действует 7 дней, повторный вход требуется реже
- Исправлен сброс токена при загрузке списка задач
- При подтверждении кода админа существующий пользователь получает роль `admin`
- Роль корректно обновляется при вводе админского кода
- Отправка формы авторизации возможна клавишей Enter
- Сборка CSS не содержит ошибок, стили работают во всех разделах
- Код проходит проверку ESLint перед запуском тестов
- Список задач кэшируется в `localStorage` на минуту для ускорения загрузки
...
- После сохранения или создания задача остаётся открытой, выбранные исполнители отображаются по имени
...
Если в логах появляется сообщение `terminated by other getUpdates request`, это значит, что бот уже запущен где-то ещё. pm2 работает в fork-режиме, но предыдущий процесс мог не завершиться. Бот теперь делает до пяти повторных попыток запуска с задержкой в три секунды. Обычно этого достаточно, чтобы старый процесс успел остановиться.
...
- Сессия использует secure cookie в продакшене, `/api/v1/tasks` ограничен rate limit
- Обзор модулей приведён в `docs/architecture.md`
- Описание масок доступа — в `docs/access_mask.md`
...
- Зависимость form-data обновлена до версии 4.0.4
