...
2. Включите опцию «Require status checks to pass before merging» и добавьте проверки `lint`, `typecheck`, `test`, `semgrep`.
3. Дополнительно отметьте статусы GitHub Apps «Codex Security», «Codex Bug», «Codex i18n» — они остаются обязательными.
4. Сохраните правило и повторите шаги для веток `work` и `staging`, если защита распространяется на них.
5. После изменения workflow обновите список обязательных статусов, чтобы защита веток отражала актуальные проверки.
...
- Лимит размера задаётся переменной `ANTIVIRUS_SIGNATURE_MAX_SIZE` (в байтах). При превышении размерности сканер блокирует файл и пишет предупреждение в `wgLogEngine`.
...
- Все статусы (активация, отключение, ошибки сканирования, обнаруженные сигнатуры) фиксируются через `wgLogEngine` и доступны в разделе логов админки.
