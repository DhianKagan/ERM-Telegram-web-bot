...
- Добавлен маршрут `/r2/sign-upload` с проверкой ключа и заголовка `Content-Length`.
- Коллекция `uploads` хранит метаданные файлов, маршрут `/r2/sign-get` проверяет владельца перед выдачей подписи.
