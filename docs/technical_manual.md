...
- При загрузке `LogisticsPage` карта добавляет векторный источник `logistics-addresses` с URL `pmtiles://tiles/addresses.pmtiles` и символ-слой `logistics-addresses-labels` с полем `housenumber`.
- Порядок слоёв фиксирован: адреса располагаются выше дорожных подписей и ниже крупных географических меток; при работе в растровом режиме (`MAP_STYLE_MODE !== "pmtiles"`) источник не подключается.
- Минимальный масштаб для отображения номеров домов — 17, размеры текста 13 pt с белым halo шириной 1.2, цвет подписи — `#0f172a`.
- При ошибке загрузки основного стиля (403/404) карта вначале переключается на штатный векторный стиль OpenFreeMap, а если он недоступен — на резервный растровый слой.
- Если в `apps/web/public/tiles` лежит `maplibre-style.json`, клиент использует его как основной стиль; при отсутствии файла применяется удалённый пресет OpenFreeMap Liberty (`https://tiles.openfreemap.org/styles/liberty`).
- Файл стиля обновляется вместе с `basemap.pmtiles` и `addresses.pmtiles`, чтобы карта и слои оставались синхронизированными.
- SSE-канал `/api/v1/logistics/events` отправляет события `route-plan.updated` с причинами `created`, `recalculated` и `updated`, поэтому все открытые `LogisticsPage` мгновенно перезагружают список задач и текущий маршрутный план после появления нового черновика или пересчёта оптимизатора.
...
![Схема слоя адресов на карте](images/logistics-addresses.svg)

### Обновление источника адресов

- Скрипт `./scripts/refresh_addresses.sh` скачивает данные `OSM_PBF_URL` и `OPENADDR_URL`, фильтрует `addr:housenumber`, конвертирует OpenAddresses при помощи встроенного Python-скрипта и формирует MBTiles через `tippecanoe`. Для работы потребуются `osmium`, `tippecanoe`, `curl`, `unzip` и `python3`.
- Масштабы регулируются переменными `MIN_ZOOM` и `MAX_ZOOM`, итоговый файл задаётся `OUTPUT_MBTILES` (по умолчанию `dist/addresses.mbtiles`). Установите `KEEP_WORKDIR=1`, если нужно сохранить промежуточные файлы.
- После генерации конвертируйте MBTiles в PMTiles командой `pmtiles convert dist/addresses.mbtiles apps/web/public/tiles/addresses.pmtiles` и замените файл в репозитории. Аналогично обновляйте `basemap.pmtiles` (см. `apps/web/public/tiles/README.md`).
- Workflow `.github/workflows/refresh-addresses.yml` запускается по понедельникам в 01:30 UTC (04:30 по Киеву, UTC+3), сохраняет `addresses.pmtiles` как артефакт и выкладывает обновлённый файл в `apps/web/public/tiles`.

### Настройка ключа стиля карты

1. Базовый пресет использует стиль OpenFreeMap Liberty и не требует ключа; при необходимости зарегистрируйтесь на [https://protomaps.com](https://protomaps.com) или другом провайдере MapLibre-совместимых стилей и создайте ключ.
2. Скопируйте URL стиля (обычно он заканчивается на `style.json?key=...`).
3. Добавьте ключ в окружение клиента и API: впишите `VITE_MAP_STYLE_URL=<URL>` в `.env` корня и перезапустите `pnpm run dev` или инфраструктуру Railway/Docker — бэкенд автоматически подхватит origin стиля и расширит CSP.
...
6. Убедитесь, что выбранный стиль доступен: при ответах 403/404 клиент переключится на OpenFreeMap, а если он тоже не загрузится — на растровый резерв; в последнем случае адресный слой и 3D-здания будут недоступны.

### Переменные окружения карты

- `VITE_MAP_STYLE_URL` — URL JSON-стиля; по умолчанию используется OpenFreeMap Liberty (`https://tiles.openfreemap.org/styles/liberty`). Можно указывать как публичный HTTPS-стиль, так и `pmtiles://` при локальном размещении.
- `VITE_MAP_STYLE_MODE` — режим рендера: `pmtiles` (вектор), `raster` (резервный слой OSM). По умолчанию определяется автоматически, но для продакшн с PMTiles оставьте `pmtiles`.
- `VITE_USE_PMTILES` — подсказка сборщику Vite для активации протокола PMTiles; значение `1` соответствует режиму `pmtiles` и используется по умолчанию в боевой сборке.
...
- `VITE_DISABLE_SSE` — переключатель SSE: `0` оставляет поток `/api/v1/logistics/events`, `1` включает поллинг с интервалом `VITE_LOGISTICS_POLL_INTERVAL_MS`.

### Проверенная связка OpenFreeMap + PMTiles

Используйте значения ниже, если подключаете OpenFreeMap Liberty и локальные адресные PMTiles:
...
VITE_USE_PMTILES="1"
VITE_DISABLE_SSE="0"
CSP_CONNECT_SRC_ALLOWLIST="https://tiles.openfreemap.org"
CSP_FONT_SRC_ALLOWLIST="https://tiles.openfreemap.org"
CSP_IMG_SRC_ALLOWLIST="https://tiles.openfreemap.org"
CSP_STYLE_SRC_ALLOWLIST="https://tiles.openfreemap.org"
```

Сервер добавляет origin стиля из `VITE_MAP_STYLE_URL` и домены Protomaps в CSP автоматически, поэтому whitelists выше достаточно для работы OpenFreeMap; дополнительные записи нужны только при выдаче статики с других доменов.
