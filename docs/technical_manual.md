...
умеет фильтровать по уровню, дате, тексту и признаку `no-csrf`, поддерживает экспорт в CSV/JSON и режим live.
Метрику `csrf_errors_total` стоит подключить к Prometheus и настроить оповещение на резкий рост.
