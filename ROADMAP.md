...
- Восстановить стабильный CI/CD: workflow Deploy и Release используют pnpm, проверяют секреты Railway и собирают проект перед выкладкой.
...
- Проверка size-limit перенесена на файлы в `apps/api/public/js`.
...
182. Маршрут `/api/v1/route-optimize` исключён из CSRF-проверки
