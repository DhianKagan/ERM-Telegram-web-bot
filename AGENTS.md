...
- Модуль `@adminjs/express` подключается статически
- Модуль `@adminjs/mongoose` подключается статически через `* as`
- AdminJS импортируется как ES-модуль для корректной работы `registerAdapter`
