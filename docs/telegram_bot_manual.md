...
- `/upload_file <taskId>` — прикрепить документ или фото к задаче и загрузить в R2. Имя файла очищается от пути
- `/upload_voice <taskId>` — прикрепить голосовое сообщение к задаче и сохранить в R2. Название также нормализуется
