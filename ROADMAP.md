...
- Подпись загрузки в R2 с проверкой размера и ключа.
- Секреты вынесены в HashiCorp Vault или AWS Secrets Manager,
  ключи обновляются по `KEY_ROTATION_CRON`, CORS R2 настраивается
  переменной `R2_CORS_ORIGIN`.
- Коллекция `uploads` сохраняет метаданные файлов, `/r2/sign-get` выдаёт подпись только владельцу.
