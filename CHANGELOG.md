...
- Форма создания задачи теперь открывает мини‑карту Google Maps через кнопку «Карта». Ссылки «Поделиться» превращаются в короткие адреса и сохраняются в полях `start_location_link` и `end_location_link`.
- API получает настройку `helmet` с директивой `frame-src https://www.google.com`, что устранило ошибку CSP при загрузке карты.
<<<<<<< codex/fix-npm-ci-sync-issue-with-package-lock.json
- Обновлён lock-файл фронтенда, исправлена ошибка 'npm ci' при сборке Docker.

=======
- Исправлена ошибка `npm ci` в Dockerfile: фронтенд обновляет `package-lock.json` через `npm --prefix bot/web install`.
- Добавлена утилита authFetch для автоматической передачи токена и редиректа на /login. Страницы не делают запросы без токена.
>>>>>>> main
