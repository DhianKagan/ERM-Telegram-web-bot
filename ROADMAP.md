...
13. Интерфейс админки на `/admin` построен на TailAdmin
14. Доступ к админке открыт только пользователям с ролью администратора
15. Авторизация осуществляется кодом из Telegram, телефон не используется
16. В коллекции `roles` хранится список прав; страница `/roles` позволяет их менять
16. Страница `/admin` содержит только раздел логов
...
108. Маршрут `/admin` объявлен как `/*splat` для Express 5
