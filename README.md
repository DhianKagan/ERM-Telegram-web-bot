...
Каталог `bot_old` содержит прежний код бота. Новый каталог `bot` пока пуст и будет собираться с нуля.
...
1. Склонируйте текущий репозиторий с модифицированным кодом:
   ...
   ```
   Исходники прежней версии находятся в `bot_old`; новый каталог `bot` создаётся пустым.
