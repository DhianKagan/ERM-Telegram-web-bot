...
const { table, nearest, clearRouteCache } = require('../src/services/route');
...
test('table формирует корректный адрес', async () => {
  ...
  expect(url.pathname).toBe('/table/v1/driving/1,1;2,2');
  expect(url.searchParams.get('points')).toBeNull();
...
test('nearest использует параметр point', async () => {
  ...
  const url = new URL(global.fetch.mock.calls[0][0]);
  expect(url.pathname).toBe('/nearest/v1/driving/1,1');
  expect(url.searchParams.get('point')).toBeNull();
