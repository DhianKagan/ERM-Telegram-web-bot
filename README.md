...
- Cookie `token` использует `SameSite=Lax` и домен из `APP_URL`, поэтому сессия не теряется при возврате с внешних сайтов. В продакшене эта cookie передаётся только по HTTPS.
