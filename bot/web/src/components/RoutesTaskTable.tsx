...
export default function RoutesTaskTable({
  tasks,
  onChange,
}: {
  tasks: Task[];
  ...
  const filtered = React.useMemo(() => {
    return tasks.filter((t) => {
      ...
      const start = t.startCoordinates
        ? `${t.startCoordinates.lat},${t.startCoordinates.lng}`
        : "";
      const finish = t.finishCoordinates
        ? `${t.finishCoordinates.lat},${t.finishCoordinates.lng}`
        : "";
      ...
      return (
        ...
        (!filters.task_type ||
          (t.task_type || "").includes(filters.task_type)) &&
      ...
      <table className="min-w-full divide-y divide-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>
            ...
            >
              Тип{" "}
              {sortBy === "task_type" ? (sortDir === "asc" ? "▲" : "▼") : ""}
            </th>
            <th
              className="cursor-pointer px-2 py-1"
              onClick={() => handleSort("startCoordinates")}
            >
              Старт{" "}
              {sortBy === "startCoordinates"
                ? sortDir === "asc"
                  ? "▲"
                  : "▼"
                : ""}
            ...
            <th
              className="cursor-pointer px-2 py-1"
              onClick={() => handleSort("finishCoordinates")}
            >
              Финиш{" "}
              {sortBy === "finishCoordinates"
                ? sortDir === "asc"
                  ? "▲"
                  : "▼"
                : ""}
            ...
            <th
              className="cursor-pointer px-2 py-1"
              onClick={() => handleSort("route_distance_km")}
            >
              Км{" "}
          ...
          <tr>
            ...
            <th className="px-2 py-1">
              <input
                ...
                onChange={(e) => changeFilter("due_date", e.target.value)}
                className="w-full rounded border px-1"
              />
            </th>
            <th className="px-2 py-1">
            ...
            <th className="px-2 py-1">
              <input
                ...
                onChange={(e) => changeFilter("task_type", e.target.value)}
                className="w-full rounded border px-1"
              />
            </th>
            <th className="px-2 py-1">
            ...
            <th className="px-2 py-1">
              <input
                ...
                onChange={(e) => changeFilter("finish", e.target.value)}
                className="w-full rounded border px-1"
              />
            </th>
            <th className="px-2 py-1">
