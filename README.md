...
- Cookie `XSRF-TOKEN` устанавливается с тем же доменом и `SameSite=Lax`,
  чтобы значение в заголовке совпадало с cookie.
- Маршрут `/api/v1/optimizer` не требует CSRF-токена.
- Тест `routeCsrf.test.js` проверяет CSRF при расчёте маршрута, а `taskFields.test.js` контролирует состав полей формы.
