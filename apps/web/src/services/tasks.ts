...
const sanitizeReason = (message: string): string =>
  stripControlCharacters(message).replace(/\s+/g, ' ').trim().slice(0, 400);
...
export const fetchTransportOptions =
  async (): Promise<TransportOptionsResponse> => {
    ...
    .then((list) =>
      Array.isArray(list)
        ? (list as User[]).map((item) => ({
            ...
            telegram_username:
              (item as User & { telegram_username?: string })
                .telegram_username ??
              item.username ??
              null,
          }))
...
export const deleteTaskDraft = async (
  kind: 'task' | 'request',
): Promise<void> => {
  const res = await authFetch(`/api/v1/task-drafts/${kind}`, {
    method: 'DELETE',
