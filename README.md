...
Каталог `bot` содержит актуальную логику бота и мини‑приложение.
Старый каталог `bot_old` удалён; исходники хранятся в истории репозитория.
`Dockerfile` в корне используется для сборки всего проекта, а `bot/Dockerfile` подходит для отдельного контейнера бота.
...
- Планировщик использует очередь с лимитом 30 сообщений в секунду
...
Исходники прежних версий удалены. Актуальная работа ведётся в каталоге `bot`.

2. Установите модуль `dotenv` для загрузки переменных окружения:
   ```bash
   npm install dotenv --prefix bot
   ```

3. Создайте файл `.env` на основе примера или существующих переменных окружения.
   Копирование примера:
   ```bash
   cp .env.example .env
   ```
  ...
  Проверьте `BOT_TOKEN` и `WEBHOOK_URL` на отсутствие опечаток, иначе Telegram может вернуть ошибку `BOT_VERIFIER_FORBIDDEN`.
  При запуске тестов также задайте `APP_URL`, иначе Jest завершится ошибкой.
  Переменные окружения должны быть установлены до импорта модулей,
  иначе `bot/src/config.js` остановит тесты сообщением
...
Административный доступ определяется по роли в Telegram‑чате, отдельные переменные логина не требуются.
...
```
По умолчанию API слушает порт 3000, а вебхук Telegraf использует `BOT_PORT=3001`.
При необходимости измените внешний порт через переменную `HOST_PORT` и подберите
другой `BOT_PORT` в `.env`, чтобы избежать конфликта.

Compose соберёт образ бота и запустит MongoDB.
Интерфейс собирается автоматически, но после изменений фронтенда
выполняйте `npm run build-client` вручную.
...
При занятости порта 3000 измените `HOST_PORT` или запустите вебхук на ином `BOT_PORT`.
...
убедитесь, что в `MONGO_DATABASE_URL` указан верный логин и пароль.
При отсутствии учётных данных он напомнит добавить их в строку подключения.
...
Dockerfile оптимизирован: сборочный слой использует рабочую директорию `/app/bot`, что сокращает повторы в логе.
...
- после сборки образы workflow удаляет `.env`, чтобы секреты не попадали в артефакты;
- автоматический деплой на Railway после успешных тестов.
- деплой через Railway CLI в релизном workflow `release.yml`;
- в `ci.yml` бэкенд устанавливает зависимости командой `npm ci --prefix bot` и запускает тесты.
...
- перед `docker compose build` выполняйте `node scripts/check_mongo.cjs` для проверки доступа к MongoDB. Скрипт берёт `mongoose` из `bot/node_modules`, если пакет не установлен глобально.
- при запуске скрипт показывает домен и имя базы без логина и пароля, что помогает убедиться в правильном URL.
...
### Ошибка `bad auth: Authentication failed`

При подключении к MongoDB может появиться сообщение `bad auth: Authentication failed`. Возможные причины:

- в секрете указан неверный пароль или логин;
- специальные символы в строке подключения не закодированы;
- IP-адрес не добавлен в разрешённый список в панели управления.

Проверить соединение можно скриптом [scripts/check_mongo.cjs](scripts/check_mongo.cjs).
...
- Устаревший каталог `bot_old` удалён, структура проекта стала компактнее.
- Удалены неиспользуемые `package.json` и `package-lock.json` в корне,
  остатки старого проекта `crystallization`.
...
- Удалена пустая директива `@apply;` в `index.css`, вызывала ошибку сборки Tailwind "Cannot apply unknown utility class".
...
- При проверке кода учитывается срок действия 5 минут, просроченные записи автоматически удаляются.
...
- Для быстрой проверки доступа к базе запустите `scripts/check_mongo.cjs`; скрипт автоматически подхватывает `.env` при отсутствии `dotenv` и ищет пакет `mongoose` в `bot/node_modules`.
- Скрипт выводит домен и имя базы перед подключением, не раскрывая логин и пароль.
...
- В inline-режиме поддерживаются команды `add <текст>` для быстрого создания задачи и `search <ключ>` для поиска до десяти задач.
  ...
  - Если токен не обладает правами Bot Verification, Telegram вернёт `BOT_VERIFIER_FORBIDDEN` и бот завершит работу.
...
- Добавлен отдельный маршрут `/`, который отдаёт `index.html` и предотвращает `403` при обращении к корню сайта.
