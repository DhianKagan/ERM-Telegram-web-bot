...
README также показывает пример запуска `./scripts/setup_tdweb.sh` перед сборкой.
 Docker Compose содержит `healthcheck` для MongoDB с командой `mongosh --eval 'db.adminCommand("ping")' -u admin -p admin --authenticationDatabase admin`, поэтому бот стартует после готовности базы.
