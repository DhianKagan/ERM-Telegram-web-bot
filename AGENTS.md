...
- Статические файлы не выдают скрытые файлы: используйте `dotfiles: 'deny'`, пути с `..` отвечают `404`.
...
- После сборки клиента копируйте `apps/web/dist` в `apps/api/public` для выдачи SPA.
