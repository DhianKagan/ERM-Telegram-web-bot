...
## В разработке
- Перенос фронтенда на React и Tailwind.
- Внедрение единой схемы валидации данных.
- Разработка middleware и сервисов для повторяющейся логики.
- Расширение тестов и настройка CI.
- Настройка шаблонов задач GitHub и добавление инструкции по вкладу.
- Подключение Swagger/OpenAPI и генерация документации API.
- Настройка Husky и линтера для проверки код-стиля.
- Базовая конфигурация ESLint размещена в `eslint.config.js`, команды `npx eslint bot/src` проверяют серверный код.
- Скрипты миграций и наполнения тестовыми данными.
- Создание админ-панели управления пользователями и ролями.
- Подключение страницы `/dashboard/tasks` к API с данными из MongoDB.
- Добавить форму создания задач в интерфейсе `/dashboard/tasks`.
- Реализовать уведомление об успешном создании задачи через `NotificationBar`.
- Добавить страницу `/dashboard/charts` с примерами графиков.
- Подготовка мануала `docs/telegram_bot_manual.md` по настройке команд и вебхуков бота.
- Исправление путей импорта компонентов на этой странице.
- Восстановление работы `/dashboard` после переноса на React.
