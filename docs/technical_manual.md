...
Этот документ собирает в одном месте актуальную информацию по настройке и запуску проекта. Исторические исследования, завершённые планы и разовые audit-материалы вынесены в `docs/archive/`, чтобы не конкурировать с каноническим набором документов.

## Канонические связанные документы

- [`../README.md`](../README.md) — корневой обзор проекта и быстрый вход в docs.
- [`docs/README.md`](README.md) — навигационный вход в документацию.
- [`docs/index.md`](index.md) — канонический список поддерживаемых документов и модель сопровождения docs.
- [`docs/architecture.md`](architecture.md) — структурный обзор monorepo и runtime-связей.
...
1. Создайте персональный токен GitHub с правами `repo`. Не сохраняйте его в shell history и не вставляйте в документацию/скриншоты.
2. Для локального повтора используйте безопасный prompt без записи в history:
   ```bash
   read -rsp "GitHub token: " DANGER_GITHUB_API_TOKEN && echo && \
   DANGER_GITHUB_API_TOKEN="$DANGER_GITHUB_API_TOKEN" pnpm danger ci
   ```

## Защита веток GitHub

1. Откройте Settings → Branches → Branch protection rules и выберите правило для `main`.
2. Включите опцию «Require status checks to pass before merging».
3. Добавьте **актуальные** проверки из `.github/workflows/*.yml` (например `CI`, `Codex Quality Gate`, `CodeQL`, `Lighthouse`). Не фиксируйте в документации устаревшие названия GitHub Apps.
4. Сохраните правило и повторите шаги для веток `work` и `staging`, если защита распространяется на них.
5. После каждого обновления workflow пересмотрите список обязательных статусов, чтобы он совпадал с текущими job.
...
- Для оценки длительных шагов сборки используйте `pnpm railway:analyze-build-log <файл_лога>`.
- Скрипт принимает данные и из `stdin`, поэтому можно передать вывод `railway logs` через пайп.
- В отчёте отображаются самые долгие операции и суммарное время по стадиям (build, stage-1 и т. д.).
...
- Историческое сравнение сценариев и ограничений сохранено в `docs/archive/routing_research.md`; для текущего состояния ориентируйтесь на этот мануал и `docs/architecture.md`.
...
Исторический план миграции из JavaScript сохранён в `docs/archive/typescript_migration_plan.md`; актуальным source-of-truth остаются кодовая база, CI и этот мануал.
...
BOT_TOKEN="$BOT_TOKEN" scripts/set_bot_commands.sh
```
...
продакшене флаг `Secure` включён автоматически; домен выставляется только
при явном `COOKIE_DOMAIN`, а без него cookie остаются host-only. В
разработке `Secure` можно отключить через `COOKIE_SECURE=false`, домен не
задаётся. Срок жизни сессии и JWT — семь дней.
...
2. Затем выполняется `/api/v1/auth/send_code` и `/api/v1/auth/verify_code`,
   после чего сервер либо выставляет legacy cookie `token` и возвращает JWT,
   либо в bearer-режиме возвращает `accessToken` и устанавливает только
   `httpOnly` refresh-cookie.
3. В bearer-режиме `accessToken` хранится только в памяти вкладки; legacy JWT
   не дублируется в отдельной cookie. `AuthContext` содержит только профиль
   пользователя и флаг загрузки.
...
5. Для защиты от XSS legacy cookie `token` имеет флаги `HttpOnly` и
   `SameSite=Lax`, а в secure-режиме — `SameSite=None`. В продакшене
   добавляется `Secure`; `Domain` выставляется только при явном
   `COOKIE_DOMAIN`, иначе cookie остаётся host-only. Токен живёт семь дней.
...
### Матрица security-практик: что уже автоматизировано, а что пока документировано

| Практика                                | Канонический документ                                                                         | Кто владеет                  | Что запускает                                                  | Что считается evidence                                                                                                  | Текущее состояние автоматизации                                                                                                                                  |
| --------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dependency audit                        | `SECURITY.md`, `docs/security/dependabot_prs.md`, `docs/security/audit_ci_false_positives.md` | Maintainer / Release manager | PR с изменением зависимостей, lockfile, образа; каждый release | Успешный `./scripts/audit_deps.sh` или эквивалентный CI run; при исключении — diff `audit-ci.json` и ссылка на advisory | **Частично автоматизировано**: `Codex Quality Gate` декларирует `pnpm run audit`, но root-script пока не подключён                                               |
| Security static analysis (ESLint rules) | `SECURITY.md`, `docs/security/cookies_csrf.md`                                                | API owner                    | PR с изменением auth / security-sensitive server code          | Лог `./scripts/security_scan.sh` или CI step с теми же правилами                                                        | **Частично автоматизировано**: workflow декларирует `pnpm lint:security`, но root-script пока не подключён                                                       |
| Semgrep / SAST                          | этот раздел + `.github/workflows/ci.yml`                                                      | Maintainer                   | Каждый PR и push в защищаемые ветки                            | GitHub Actions job `semgrep`                                                                                            | **Автоматизировано в CI** через job `semgrep`                                                                                                                    |
| CodeQL                                  | этот раздел + `.github/workflows/codeql.yml`                                                  | Maintainer                   | PR, push, weekly schedule                                      | GitHub Code Scanning alerts / успешный workflow run                                                                     | **Автоматизировано в CI**                                                                                                                                        |
| Cookies / CSRF                          | `docs/security/cookies_csrf.md`                                                               | API owner                    | Любой PR с auth/cookie/proxy изменениями                       | Тесты, smoke `GET /api/v1/csrf`, заголовки `Set-Cookie`, ожидаемый `403` без CSRF header                                | **Частично автоматизировано**: покрывается тестами, но manual smoke остаётся обязательным                                                                        |
| CSP                                     | `docs/security/csp.md`                                                                        | Web owner + API owner        | Любой PR с новыми внешними источниками/allowlist               | Diff allowlist, проверка CSP header, violation reports                                                                  | **Документировано**: отдельной CI-проверки сейчас нет                                                                                                            |
| Secrets hygiene                         | `SECURITY.md`, `INCIDENT_RESPONSE.md`                                                         | Author PR + Maintainer       | Каждый PR/release; любой инцидент ротации                      | Отсутствие секретов в diff, запись о ротации в incident ticket/postmortem                                               | **Частично автоматизировано**: `pnpm run docs:audit` проверяет docs security hygiene, но отдельного repo-wide secret-scanning workflow для всего дерева пока нет |
| Incident response path                  | `SECURITY.md`, `INCIDENT_RESPONSE.md`                                                         | Incident commander           | Подтверждённый security incident                               | Incident ticket, лог ротации, deploy evidence, postmortem                                                               | **Ручной runbook**                                                                                                                                               |

### Обязательный short list для PR / release

1. **Dependencies:** прогнать dependency audit и зафиксировать advisory / allowlist-решение.
2. **CSP:** при добавлении новых доменов приложить diff allowlist и проверку итогового CSP header.
3. **CSRF / cookies:** подтвердить `GET /api/v1/csrf`, cookie flags и отрицательный тест без `X-XSRF-TOKEN`.
4. **Secrets:** убедиться, что в diff, workflow и логах нет секретов; при ротации создать incident ticket.
5. **Incident response path:** для любого security-блокера сослаться на `INCIDENT_RESPONSE.md` и зафиксировать owner инцидента.
...
Дополнительный исторический follow-up после аудита хранится в `docs/archive/apply_analysis_plan.md`.
