...
Каталог `bot` содержит основную логику Telegram‑бота и мини‑приложение.
Каталог `admin` удалён, весь функционал реализован в мини‑приложении.
Каталог `bot_old` удалён, его функции полностью перенесены в `bot`.
...
- Автоматизация генерации `.env` скриптом `create_env_from_exports.sh`, который
  ...
  `.env.example`.
- Учётная запись администратора авторизуется через Telegram ID, переменные `ADMIN_EMAIL` и `ADMIN_PASSWORD` больше не используются.
...
- В инструкции по установке теперь явно указано, что `npm ci --prefix bot || npm --prefix bot install` автоматически собирает веб‑модуль.
...
- После создания `.env` workflow выводит `MONGO_DATABASE_URL` без пароля и останавливается при неверном формате.
- Далее выполняется `grep -E '^MONGO_DATABASE_URL|^APP_URL' .env`, чтобы убедиться в правильной подстановке переменных.
...
- После сборки образов workflow удаляет `.env`, чтобы секреты не попадали в артефакты.
- Перед подключением скрипт выводит домен и имя базы без логина и пароля, что ускоряет поиск ошибок.
...
- Добавлена переменная `BOT_PORT` для запуска вебхука Telegraf на отдельном порте и устранения конфликта.
- Удалены устаревшие `package.json` и `package-lock.json` из корня проекта,
  принадлежавшие старому модулю кристаллизации.
...
- Если Bot API возвращает `BOT_VERIFIER_FORBIDDEN`, модуль `verify.js` завершает работу и просит проверить токен.
...
- Удалена пустая директива `@apply;` в `bot/web/src/index.css`, из-за которой Tailwind завершал сборку с ошибкой.
...
- Добавлен маршрут `/`, который отдаёт главный файл `index.html` и тем самым исключает 403 при обращении к корню.
...
- Проверено подключение к кластеру `shinkansen.proxy.rlwy.net` с базой `agromarket`; для проверки используется скрипт `scripts/check_db_fetch.cjs`.
...
- Скрипт сообщает домен и имя базы, не включая логин и пароль, чтобы оперативно проверить адрес.
...
- Страница `/chats` подключает TDWeb (TDLib в WebAssembly) через файлы из `bot/web/public/tdlib`. Скрипт `scripts/setup_tdweb.sh` устанавливает их автоматически.
