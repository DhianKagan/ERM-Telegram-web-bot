...
87. POST `/api/v1/logs` позволяет добавить собственное сообщение
88. Исправлена ошибка линтера в `TaskDialog`: состояние `selectedAction` подсвечивает выбранную кнопку
89. Сервис ORSM поддерживает `/table`, `/nearest`, `/match` и `/trip`
90. Координаты запросов к маршрутам проходят валидацию
