...
5. Для bearer auth проверяйте parity env-флагов: `AUTH_BEARER_ENABLED == VITE_AUTH_BEARER_ENABLED` (оба `true`/`1`). Если `api` и `web` в одном Railway-сервисе — оба флага задаются в нём; если сервисы раздельные — синхронизируйте флаги в соответствующих сервисах.

## 6. Почему Railway помечает деплой как Failed

Railway помечает деплой как «Failed», если контейнер стартовал, но сервис не проходит базовые проверки доступности или завершается с ошибкой. Чаще всего это связано со следующими причинами:

1. **Нет ответа на health-check.** По умолчанию Railway обращается к корневому пути `/` и ожидает HTTP 200. Если маршрут отсутствует или возвращает 404/500, деплой будет считаться неуспешным, даже если процессы запущены. В проекте корневой `GET /` отвечает `{ "status": "ok" }`, а `/health` возвращает расширенную диагностику.
2. **Неправильный порт.** Railway передаёт порт через переменные окружения (`PORT`/`RAILWAY_TCP_PORT`). Если сервер слушает захардкоженный порт, сервис не станет доступным. Не задавайте `PORT` и `HOST_PORT` вручную в Railway — используйте порт платформы.
3. **Ненулевой код выхода в скриптах запуска.** Любая ошибка в release‑шаге или стартовом скрипте приводит к коду выхода ≠ 0, и Railway фиксирует «Failed». Проверьте `Procfile.railway`, `Procfile` и вспомогательные скрипты (например, установки команд бота), чтобы они не падали из‑за временных ошибок.
4. **Фоновый запуск процессов.** Если основной процесс контейнера завершается (например, `pm2` запущен без `pm2-runtime`), Railway считает сервис упавшим. Для нескольких процессов используйте `pm2-runtime` и следите, чтобы `api` действительно слушал нужный порт.

## 7. Как исправить

1. Убедитесь, что `GET /` отвечает 200 и возвращает JSON статуса, а `GET /health` доступен без авторизации.
2. Удалите пользовательские значения `PORT` и `HOST_PORT` в Railway и перезапустите сервис, чтобы подхватился порт платформы.
3. Проверьте, что release‑шаг (`pnpm build` и пост‑скрипты) завершается успешно. Ошибки сборки или миграций всегда помечают деплой как «Failed».
4. Проверьте логи Railway на наличие таймаута health‑check или падения процесса. При необходимости используйте автоматизированный сбор логов из [railway_logs.md](./railway_logs.md).
