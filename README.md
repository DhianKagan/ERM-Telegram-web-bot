...
2. Укажите рабочую директорию `bot` и версию Node `18`.
3. Скопируйте переменные окружения из `.env.example`.
4. В корне проекта добавлен `Procfile` с командой `web: npm --prefix bot start`.
   Pella автоматически использует его для запуска приложения.
