...
3. Создайте файл `.env` на основе примера или существующих переменных окружения.
  ...
  Если виджет вовсе не появляется, убедитесь, что домен из `APP_URL` разрешён в
  разделе **Web Apps** в BotFather, а переменные `BOT_USERNAME` и
  `VITE_BOT_USERNAME` указаны верно.
