...
## Интерфейс
Мини‑приложение основано на Vite и Tailwind. Боковое меню и шапка реализованы через упрощённые компоненты TailAdmin. Стартовая страница `/dashboard` отображает приветствие и в дальнейшем будет подключена к API.
...
### Итог
Проект представляет готовую базу и быстро разворачивается, но требует модернизации для полноценного продакшна. Интерфейс построен на React + Vite + Tailwind, использует упрощённые компоненты TailAdmin для разделов Dashboard, Tasks, Logs и Roles. Код фронтенда форматируется Prettier.
