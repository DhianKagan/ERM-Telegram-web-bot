...
Маршрут `/r2/sign-upload` выдаёт подпись для загрузки в R2.
`key` проверяется по регулярному выражению, размер файла
ограничен заголовком `Content-Length`.
...
Dependabot еженедельно обновляет npm-зависимости и помечает PR меткой `security`; шаг `audit-ci` в CI падает при уязвимостях уровня high.
