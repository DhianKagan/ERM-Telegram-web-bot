...
2. Создайте файл `.env` на основе примера или экспортированных переменных.
   Для копирования примера выполните:
   ```bash
   cp .env.example .env
   ```
   Либо сгенерируйте `.env` из переменных окружения скриптом:
   ```bash
   ./scripts/create_env_from_exports.sh
   ```
   Скрипт читает список переменных из `.env.example` и записывает значения из
   вашего окружения.
