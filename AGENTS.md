...
> ## Assistant runtime (для чат-агента)
>
> **Источник истины для агента:** `.openai/assistant_instructions.json` (машинно-читаемо). Этот `AGENTS.md` дополняет файл и описывает контекст репозитория.
>
> **Перед началом любой задачи агент обязан:**
>
> 1. проверить read-доступ к репозиторию и открыть `AGENTS.md` и `.openai/assistant_instructions.json`;
> 2. действовать по `workflow` и `patch_policy` из `.openai/assistant_instructions.json`;
> 3. использовать **pnpm** для всех команд; не создавать `package-lock.json`.
>
> **Формат ответа агента** (всегда в таком порядке):
>
> - `Mode: <Lite|Standard|Research+>`; Timezone: **Europe/Kyiv**
> - **Plan** — цели, допущения (≤3), риски (≤3)
> - **Commands** — детальные, **без разрушительных**
> - **Changes** — структурированный список изменений по файлам
> - **PR** — что и почему, DoD-чек-лист, выдержки логов, самопроверка (≤5), совместимость
>
> **Патчи:** генерировать `.cjs`-патчи в `patch_cjs/`, оркестратор `patch_cjs/apply-all.cjs`, затем `pnpm install` и `pnpm -F web run prebuild`.  
> **Цитирование:** до 5 фактов с источником и датой в формате `【source, YYYY-MM-DD】`.  
> **Безопасность:** не исполнять разрушительные команды и не обрабатывать секреты в явном виде.
>
> См. `.openai/assistant_instructions.json` (версия и правила синхронизируются при изменениях).

## Оглавление

1. [Стиль и архитектура](#стиль-и-архитектура)
2. [Качество и тесты](#качество-и-тесты)
3. [Документация](#документация)
4. [Обзор архитектуры](#обзор-архитектуры)
5. [Контракты модулей](#контракты-модулей)
6. [Инварианты системы](#инварианты-системы)
7. [Список «не трогать»](#список-не-трогать)
8. [Согласованные паттерны](#согласованные-паттерны)
9. [Шаблон промпта для вкладки «Проверка кода»](#шаблон-промпта-для-вкладки-проверка-кода)
...
- Сборка клиента проверяется `pnpm size`; перед запуском соберите фронтенд `pnpm --filter web run build:dist`, лимит для `apps/web/dist/*.js` — 200 KB.
...
- ⚠️ Требование Codex: актуализируйте диаграммы и текст в `docs/architecture.md` при изменении любого элемента архитектуры.
- Клиентская часть: SPA на React/Vite в `apps/web`, поддержка Telegram WebApp SDK опциональна, роутинг через React Router с ленивой загрузкой.
- Серверная часть: NestJS API в `apps/api`, REST на префиксе `/api/v1`, база MongoDB через Mongoose, фоновые задачи на BullMQ.
- Интеграции: Telegram-бот из `apps/bot` использует сервисный токен и обращается к API, мониторинг собирает Prometheus (каталог `prometheus/`) и профилирование в `profiling/`.
- Инфраструктура: Docker/Nixpacks описаны в корне, CI/CD используют `scripts/` и конфигурации `codex/`.
- Общие пакеты: разделены в `packages/`, типы и утилиты переиспользуются клиентом, сервером и ботом.

## Контракты модулей

- ⚠️ Требование Codex: при изменении контракта обновляйте типы, тесты и документацию в `docs/architecture.md`.
- `apps/api/src/modules/tasks`: контроллеры принимают DTO из `apps/api/src/dto`, возвращают объекты `{id,status,assignee,history,custom}`, изменение статуса допускается только через `TasksService.updateStatus`.
- `apps/web/src/modules/tasks`: хук `useTaskForm` обязан возвращать `{formVersion, fields, submit}` и обеспечивать динамический рендер `custom` через реестр виджетов.
- `apps/bot/src/scenes`: каждая сцена реализует `enter`, `leave`, `handle` и использует `SessionContext`; сетевые вызовы выполняются через `apiClient` с ретраями и логированием ошибок.
- `packages/shared/src/permissions`: экспортирует `hasMask`, `combineMasks`, `maskFromRole`; новые роли проходят через эти функции, маски не хардкодятся в клиентах.
- `scripts/db/ensureIndexes.ts`: возвращает `Promise<void>`, допускает многократный запуск без побочных эффектов.
- Новые публичные модули документируются и получают тесты-контракторы.

## Инварианты системы

- ⚠️ Требование Codex: нарушение любого инварианта считается блокирующим и требует фикса до релиза.
- Маски доступа соответствуют матрице `docs/permissions.md`; маска 8 выдаётся только роли admin.
- Задачи содержат `formVersion`, `history`, `custom` (возможно пустой объект); отсутствие полей приводит к отказу сохранения.
- Аудит записывает изменения статусов задач, файлов и профилей и не допускает правок записей.
- Кеш OSRM сбрасывается при обновлении задач; TTL управляется `ROUTE_CACHE_TTL`, отклонения фиксируются алертами.
- Лимитер Telegram использует `telegram_id`, капча в `X-Captcha-Token` — единственный способ обхода ограничений.
- В бою обязательно включён HTTPS и заголовок `Strict-Transport-Security` через nginx конфигурацию.

## Список «не трогать»

- ⚠️ Требование Codex: изменения в перечисленных объектах допускаются только после согласования с владельцами компонентов.
- Автогенерация (`apps/api/src/generated/**`, `packages/types/generated/**`) — правки только через скрипты генерации.
- Скрипты безопасности `scripts/check_mongo.mjs`, `scripts/audit_deps.sh` — не удалять проверки и не упрощать логику.
- Конфигурации мониторинга в `prometheus/` и `profiling/` — менять только совместно с SRE.
- `.env.example` — редактировать после согласования с инфраструктурой.
- Материалы `docs/archive/` — неизменяемы, допускаются только обновления ссылок при переносе файлов.

## Согласованные паттерны

- ⚠️ Требование Codex: новые реализации следуют паттернам и фиксируют отклонения в `docs/architecture.md`.
- Обработка ошибок: используем `AppError`/`ApiError`, на клиент отдаём Problem Details с локализацией.
- Состояние клиента: React Query с ключами `[domain, id]`, оптимистические апдейты выполняются только при наличии стратегии отката.
- Логирование: Winston с `requestId` и `userId`, уровень по умолчанию `info`, ошибки — `error`, trace передаётся в Prometheus.
- Структура файлов: модульная структура `feature/{components,hooks,services,model,ui}`, UI-атомы из `packages/ui`.
- Тестирование: e2e — Playwright с фикстурами из `tests/utils`, API — Supertest, внешние сервисы мокируются `nock`.
- CI: новые шаги добавляются в `codex/config.yml` и GitHub Actions с указанием условий запуска и уведомлений.
- Документация: все изменения потоков данных сопровождаются обновлением диаграмм PlantUML.

## Шаблон промпта для вкладки «Проверка кода»

При проверке изменений используйте следующий шаблон, адаптируя примеры под конкретный PR. Справочник доступен в [`CODE_REVIEW_GUIDE.md`](CODE_REVIEW_GUIDE.md).
