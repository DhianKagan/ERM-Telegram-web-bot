...
ошибки могут дублироваться в Telegram. Интерфейс логов позволяет фильтровать по уровню, тексту и дате и сортировать записи.
...
Тесты `routeCsrf.test.js` и `taskFields.test.js` помогают обнаруживать проблемы с CSRF и валидностью полей формы.
## Карта запросов
...
Для устойчивости к перезапускам сохраните данные Redis на общем томе
