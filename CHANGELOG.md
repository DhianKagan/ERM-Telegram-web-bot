...
- Конфиг API принимает `MONGO_URL`/`MONGO_PUBLIC_URL`, автоматически добавляет `MONGO_DATABASE_NAME` и `authSource=admin`; обновлены примеры окружения и инструкции Railway.
- Восстановлены workflow GitHub Actions: CodeQL локализован, деплой и релиз проверяют секреты Railway и собирают проект через pnpm.
- Добавлен отдельный чек-лист перед PR (`docs/pre_pr_checklist.md`), README и docs/README ссылаются на него.
