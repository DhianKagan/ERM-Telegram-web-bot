...
7. Проверка MongoDB скриптом `check_mongo.cjs`, использование healthcheck в Docker
8. Поддержка pm2 6.0.8 и повторных подключений к базе
9. Настройка fork-режима pm2 для предотвращения ошибок polling
10. Синхронизация команд и сообщений скриптами при деплое
10. Расширение API и интерфейса по мере необходимости
11. Пагинация списка задач для уменьшения нагрузки на клиент
12. Автоматическое формирование `slug` для человекочитаемого URL
13. Улучшение SEO и доступности (robots.txt, aria‑label, meta description)
14. Расширенная модель задачи с логистикой, закупками и работами
15. Поддержка enum-полей для типов транспорта, оплаты и статусов
...
79. Для каждого оптимизированного маршрута создаётся ссылка Google Maps
    с максимум 10 точками и выводится в отдельном окне
80. Если выбрано несколько машин, задачи распределяются между всеми авто
