...
После команды `/start` бот сразу отправит ссылку на мини‑приложение, поэтому
использование `/app` для входа больше не обязательно.
...
Для доступа к API или мини‑приложению можно зарегистрироваться напрямую через
HTTP-запрос:
...
В ответ придёт JWT, который следует передавать в заголовке `Authorization` при
дальнейших запросах.
