...
* **Сворачивание Sidebar на десктопе:** добавьте отдельное состояние `collapsed` и кнопку с иконками `ChevronDoubleLeft`/`ChevronDoubleRight`. В свернутом режиме Sidebar имеет ширину `w-20`, текст скрывается, остаются только иконки.
