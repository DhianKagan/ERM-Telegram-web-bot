...
function isMessageNotModifiedError(error) {
    ...
    const response = responseRaw && typeof responseRaw === 'object'
        ? responseRaw
        : null;
    const descriptionSource = typeof response?.description === 'string'
...
function isMessageMissingOnEditError(error) {
    ...
    const record = error;
    const errorCode = typeof record.response?.error_code === 'number'
    ...
    const descriptionSource = typeof record.response?.description === 'string'
...
const escapeHtml = (value) => String(value).replace(htmlEscapePattern, (char) => htmlEscapeMap[char] ?? char);
...
async function loadCancelRequestContext(taskId, actorId) {
    ...
    const identifier = (0, taskMessages_1.getTaskIdentifier)(plain) || taskId;
    const docId = typeof plain._id === 'object' &&
        plain._id !== null &&
        'toString' in plain._id
        ? plain._id.toString()
        : String(plain._id ?? taskId);
...
async function showMainMenu(ctx) {
    await ctx.reply(messages_1.default.menuPrompt, telegraf_1.Markup.keyboard([
        ['Регистрация в ERM'],
        ['ERM веб-клиент'],
    ]).resize());
}
async function checkAndRegister(ctx) {
    ...
    if (user) {
        ...
        await (0, service_1.createUser)(ctx.from.id, ctx.from?.username || '');
...
const extractTelegramErrorCode = (error) => {
    ...
    const responseCode = typeof record.response?.error_code === 'number'
        ? record.response.error_code
        : null;
    return responseCode;
};
...
const buildDirectTaskMessage = (task, link, users, appLink = null, options) => {
    ...
    const startLocation = typeof task.start_location === 'string'
...
async function refreshTaskKeyboard(ctx, taskId, snapshot) {
    let context = snapshot ?? { plain: null, users: {} };
    if (!snapshot) {
        try {
            const taskDoc = await (0, service_1.getTask)(taskId);
            if (taskDoc) {
                const plainSource = typeof taskDoc.toObject === 'function'
                    ? taskDoc.toObject()
                    : taskDoc;
    ...
    const chatId = resolveChatId();
    const link = (0, messageLink_1.default)(chatId, messageId ?? undefined, topicId ?? undefined);
    if (ctx.chat?.type === 'private') {
        const appLink = plain ? (0, taskLinks_1.buildTaskAppLink)(plain) : null;
        const keyboard = (0, exports.buildDirectTaskKeyboard)(link, appLink ?? undefined);
...
async function denyCancellation(ctx, taskId, message) {
    ...
    await ctx.answerCbQuery(message ?? messages_1.default.taskCancelForbidden, {
        show_alert: true,
    });
}
...
const registerTextHandler = exports.bot.on?.bind(exports.bot);
