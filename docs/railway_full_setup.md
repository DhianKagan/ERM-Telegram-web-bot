...
3. Railway автоматически использует `Procfile`, который собирает клиент и запускает pm2.
   Файл `railway.json` закрепляет работу через `nixpacks.toml` и команду запуска `pm2-runtime`, поэтому настройки в CI и в автодеплое остаются синхронизированными.
