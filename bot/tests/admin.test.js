...
jest.mock('../src/api/middleware', () => ({
  verifyToken: (req, _res, next) => { req.user = { role: req.headers['x-role'] } ; next() },
  checkRole: role => (req, res, next) => req.user.role === role ? next() : res.sendStatus(403),
  asyncHandler: fn => fn,
  errorHandler: (err, _req, res, _next) => res.status(500).json({ error: err.message })
}))

const { listLogs, writeLog, listRoles, updateRole } = require('../src/services/service')
const { verifyToken, checkRole, asyncHandler } = require('../src/api/middleware')
...
app.post('/api/v1/logs', verifyToken, asyncHandler(async (req, res) => {
  if (typeof req.body.message === 'string') await writeLog(req.body.message)
  res.json({ status: 'ok' })
}))
app.get('/api/v1/roles', verifyToken, checkRole('admin'), asyncHandler(async (_req, res) => {
  res.json(await listRoles())
}))
app.patch('/api/v1/roles/:id', verifyToken, checkRole('admin'), asyncHandler(async (req, res) => {
  res.json(await updateRole(req.params.id, req.body.permissions))
}))

test('получение логов доступно админу', async () => {
  const res = await request(app).get('/api/v1/logs').set('x-role','admin')
  expect(res.body[0].message).toBe('log')
})

test('обычный пользователь получает 403 при доступе к логам', async () => {
  const res = await request(app).get('/api/v1/logs').set('x-role','user')
  expect(res.status).toBe(403)
})

test('запись лога вызывает writeLog', async () => {
  await request(app).post('/api/v1/logs').set('x-role','admin').send({ message: 'm' })
  expect(writeLog).toHaveBeenCalledWith('m')
})

test('получение ролей', async () => {
  const res = await request(app).get('/api/v1/roles').set('x-role','admin')
  expect(res.body[0].name).toBe('admin')
})

test('обновление роли', async () => {
  const res = await request(app).patch('/api/v1/roles/1').set('x-role','admin').send({ permissions:['tasks'] })
