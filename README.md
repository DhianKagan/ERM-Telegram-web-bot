...
2. Укажите версию Node `18`.
3. Рабочая директория должна совпадать с местом расположения `Procfile`. По
   умолчанию файл находится в корне репозитория и содержит команду
   `web: npm --prefix bot start`. Если планируется использовать директорию `bot`
   в качестве рабочей, разместите в ней отдельный `Procfile` с той же
   командой.
...
2. Тип процесса определяется автоматически из `Procfile`, дополнительная настройка не требуется.
3. Перенесите значения из `.env.example` в раздел переменных окружения.
4. Сохраните настройки и запустите деплой. Через пару минут сервис станет доступен.
