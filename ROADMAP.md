...
Каталог `bot` теперь содержит рабочий код, который изначально находился в `bot_old`.
`bot_old` оставлен как резервная копия для истории изменений.
...
- Перед сборкой Docker в workflow выполняется `node scripts/check_mongo.cjs` для быстрой проверки подключения к MongoDB.
...
- Добавлен скрипт `scripts/check_mongo.cjs` для быстрого ping к MongoDB.
