...
- Исправлена сборка: пакет shared экспортирует типы логистики и аналитики, конфигурация GraphHopper считывает переменные окружения.
...
- Size-limit проверяет бандлы `apps/web/dist/*.js` с ограничением 200 KB, что предотвращает ложные срабатывания.
...
- Лимит size-limit для `apps/web/dist/*.js` установлен на 200 KB, чтобы предотвратить рост бандла и падение CI.
...
- В примерах `.env` и документации `ROUTING_URL` указывает на `https://router.project-osrm.org`
