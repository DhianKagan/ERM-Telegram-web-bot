...
Каталог `bot` включает код сервера и мини‑приложения.
Каталог `bot_old` был удалён, при необходимости изучайте историю репозитория.
...
- В этой же секции подчёркнуто, что `npm ci --prefix bot || npm --prefix bot install` автоматически собирает веб‑модуль.
...
- Используется модуль `dotenv` для загрузки `.env`; установите его командой `npm install dotenv --prefix bot`.
...
- Для вебхука предусмотрена переменная `BOT_PORT`, по умолчанию 3001; задайте её, если API использует тот же порт.
...
- В API присутствует маршрут `/`, который отдаёт `index.html` и предотвращает ошибку `403` при обращении к корню сайта.
...
- Поддерживайте подразделы устранения ошибок в README, добавляя типичные проблемы и пути их решения.
- При добавлении маршрутов используйте `helmet`, `cors` и валидацию через `express-validator`.
- `/auth/register` и `/api/auth/telegram` проверяют входные данные через `express-validator`.
...
- Следующий шаг выводит `MONGO_DATABASE_URL` без пароля и завершает workflow при некорректном формате строки подключения.
- Затем выполняется `grep -E '^MONGO_DATABASE_URL|^APP_URL' .env`, что упрощает диагностику ошибок подключения.
- По завершении workflow удаляйте `.env`, чтобы секреты не сохранялись в артефактах.
...
- График `TasksChart` оформлен в корпоративных цветах.
...
- Ошибка `Cannot apply unknown utility class` возникает из-за пустой директивы `@apply;` в CSS. Удалите строку, как в `bot/web/src/index.css`.
...
- Цветовая палитра упрощена: удалены `--color-blue-light-*` и `--color-orange-*`,
  ...
  - Убедитесь, что домен из `APP_URL` добавлен в BotFather в разделе `Web Apps`, иначе виджет Telegram Login не отобразится.
...
- Для установки TDWeb (TDLib в WebAssembly) используйте скрипт `scripts/setup_tdweb.sh`, который копирует файлы в `bot/web/public/tdlib`.
...
- В Dockerfile финального слоя задавайте `NODE_ENV=production`, чтобы не ставить dev-зависимости.
- Вход администратора осуществляется через Telegram ID, переменные `ADMIN_EMAIL` и `ADMIN_PASSWORD` удалены.
- Удалены `package.json` и `package-lock.json` из корня; эти файлы относились к
  устаревшему модулю кристаллизации.
