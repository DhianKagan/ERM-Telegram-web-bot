...
4. Для защиты от XSS cookie `token` имеет флаги `HttpOnly` и `SameSite=Lax`. В продакшене добавляется `Secure`, в режиме разработки cookie передаётся и по HTTP. Токен живёт семь дней и совпадает по домену с `APP_URL`.
