...
устанавливает cookie `XSRF-TOKEN` и возвращает значение токена. Клиент берёт
его из cookie и передаёт в заголовке `X-XSRF-TOKEN`. При ошибке 403 запрос
токена можно повторить.
