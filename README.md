...
```
Если сборка завершается ошибкой отсутствия модулей, установите зависимости:
```bash
npm --prefix bot/web install > /tmp/npm_install.log 2>&1 && tail -n 20 /tmp/npm_install.log
```
Затем повторите сборку. Убедитесь, что каталог `bot/public` содержит статические файлы, и проверьте логи на отсутствие ответов `404`.
...
Vite создаёт каталоги с `index.html`, поэтому путь `/dashboard` отвечает без ошибки 404 при прямом обращении.
...
### Плюсы
- Фронтенд переписан на React + Vite с Tailwind.
- Swagger доступен на `/api-docs`.
- Настроены Jest и Supertest, Husky выполняет тесты и линт перед коммитом.
- Есть CI на GitHub Actions и скрипты миграций/сидов.
- Чёткая бизнес-логика и REST API, пример `.env` и Docker‑окружение.

### Минусы
- Пока мало защитных middleware и валидации.

### Как улучшить
- Доработать защиту API (helmet, cors, лимитеры).
- Расширить документацию ролей и процессов.
