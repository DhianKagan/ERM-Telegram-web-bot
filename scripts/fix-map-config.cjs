// scripts/fix-map-config.cjs
const fs = require('fs');
const path = require('path');

const fp = path.resolve('apps/web/src/config/map.ts');

// Полный набор экспортов, которых ждёт твой код (совместим с Protomaps v5)
const content = `// Map config unified for MapLibre + Protomaps CDN
