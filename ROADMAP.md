...
47. Убрать поддержку вебхуков и использовать только polling.
48. Проверять статус пользователя через `getChatMember` при верификации кода.
49. Сделать меню бота минимальным: в BotFather оставить только `/start`.
50. Реализовать команду `/register` и проверку участия в группе.
51. Отправлять токен доступа только в личные сообщения.
52. Позволить пользователю редактировать ФИО и телефон в профиле.

53. Вынести поля формы задачи в общий модуль bot/shared/taskFields.cjs.
54. Исправить относительный путь импорта taskFields.cjs в TaskForm.
55. Объединить формы создания задачи в единый компонент TaskForm и использовать
    его во всех частях приложения.
56. Реализовать главное меню без слешей и обработчики `bot.hears`, описать раздел в telegram_bot_manual.md.
