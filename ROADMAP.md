...
14. Модуль `@adminjs/express` подключается статически
15. Модуль `@adminjs/mongoose` импортируется через `* as` для ES‑модулей
16. Импорт AdminJS осуществляется как ES‑модуль для корректной работы `registerAdapter`
