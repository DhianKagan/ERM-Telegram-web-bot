...
Вложения сохраняются в подкаталоге пользователя внутри каталога `STORAGE_DIR` (по умолчанию `apps/api/public`) и содержат метаданные: автор, дата загрузки, тип и размер. Список файлов хранится в коллекции `File` и поддерживает фильтрацию по пользователю и типу.
Панель администратора содержит раздел «Файлы» для просмотра и удаления вложений.
Библиотека Chonky для этого раздела поставляется со встроенными стилями, отдельные CSS-файлы не подключаются.
...
Скачивание вложений происходит по защищённому маршруту `/api/v1/files/:id`, требующему авторизации. Свободное место на диске экспортируется метрикой `disk_free_bytes`; при нехватке места администраторам отправляется уведомление.
