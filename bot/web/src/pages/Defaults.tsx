...
import { fetchDefaults, updateDefaults } from "../services/dicts";
      ...
      <ul className="space-y-2">
        {values.map((v, i) => (
          <li key={i} className="flex gap-2">
            ...
            />
            <button onClick={() => setValues(values.filter((_, idx) => idx !== i))} className="btn">Удалить</button>
          </li>
        ))}
      </ul>
      <button onClick={() => setValues([...values, ""])} className="btn">Добавить</button>
