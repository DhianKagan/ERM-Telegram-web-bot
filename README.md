...
- Поддерживаются enum `task_type`, `transport_type`, `payment_method`, `priority`, `status` (добавлено значение `canceled`)
...
- Кнопки «Принять», «Выполнено», «Изменить», «Отменить» находятся внизу формы и становятся зелёными при соответствующем статусе
...
- Команда `/task_form_app` открывает форму новой задачи в мини‑приложении
- Команда `/my_tasks` выводит задачи, в которых пользователь участвует в любой роли
