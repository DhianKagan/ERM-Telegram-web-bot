...
- Статическую страницу `index.html` проверяйте на контраст командой `pnpm a11y`.
...
- Переменная `STORAGE_DIR` задаёт базовый каталог статических файлов и вложений (по умолчанию `bot/public`).
