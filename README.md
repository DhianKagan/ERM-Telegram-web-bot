...
4. Запустите контейнеры:
  ...
  Если команда завершается ошибкой `lstat /client: no such file or directory`,
  убедитесь, что запуск происходит из корня репозитория и каталог
  `bot/client` существует.
...
### Минусы
- Валидация данных минимальна, API можно сломать неверными запросами.
- Требуются дополнительные меры защиты (helmet, cors, лимитеры).
- Нужно расширять тесты и поддерживать покрытие.
- Документация API пока поверхностная.
- Скрипты миграций и сидов ограничены базовыми примерами.

### Как улучшить
- Расширить защиту API (helmet, cors, лимитеры).
- Подготовить полноценные сценарии бизнес‑процессов и расширить документацию.
- Усложнить миграции MongoDB и настроить деплой через CI/CD.
- Разработать дополнительные разделы админ‑панели (логи, роли).
