...
`scripts/check_mongo.cjs` проверяет доступность MongoDB. Скрипт автоматически читает `.env` даже без модуля `dotenv`.
...
- перед `docker compose build` выполняйте `node scripts/check_mongo.cjs` для проверки доступа к MongoDB (загрузка `.env` работает без `dotenv`).
...
Для быстрой проверки доступа к базе запустите `scripts/check_mongo.cjs`; скрипт автоматически подхватывает `.env` при отсутствии `dotenv`.
