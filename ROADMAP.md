...
- Восстановлена типизация логистики: shared экспортирует события и метрики, GraphHopper настраивается через переменные окружения.
...
- Документация по маскам доступа и ролям вынесена в `docs/permissions.md`.
- Реализованы CRUD‑роуты для флотов, департаментов, сотрудников, должностей и отделов.
- Добавлена модель CollectionItem с поиском и пагинацией через репозиторий.
- Управление департаментами, отделами, должностями, сотрудниками и автопарком с поиском и пагинацией вынесено на страницу `/cp/settings`,
...
- Настроена публикация отчётов Lighthouse CI через GitHub App без собственного сервера.
...
- Оптимизировать размер клиентского бандла; текущий лимит size-limit составляет 200 KB для `apps/web/dist/*.js`.
- Проверка size-limit выполняется по файлам `apps/web/dist/*.js`.
- Перевести таблицы на React Table с серверной пагинацией, скрытием и перестановкой колонок, экспортом CSV/XLSX/PDF в общем тулбаре.
- Маршрут SPA использует `app.get('*')` для отдачи `/login` и других путей.
- Middleware `verifyToken` перенаправляет на `/login` при отсутствии авторизации.
- Команда `pnpm a11y` запускает `vite preview` и проверяет контрастность `http://localhost:4173/index.html` через `@axe-core/cli`.
...
- Вложения задач сохраняются в подкаталоге пользователя на локальном томе (`STORAGE_DIR`, по умолчанию `apps/api/uploads`) и включают метаданные: автор, дата, тип и размер.
- Хранилище файлов использует коллекцию `File` и поддерживает фильтрацию по `userId` и типу.
- Реализована drag-and-drop загрузка нескольких файлов с chunk-upload.
- Скачивание файлов выполняется через защищённый маршрут `/api/v1/files/:id`; мониторится свободное место на диске и отправляются предупреждения администраторам.
- Вложения сканируются антивирусом, загрузки и скачивания пишутся в журнал, действуют лимиты количества и объёма на пользователя.
- Сигнатурный антивирус включён по умолчанию (`ANTIVIRUS_VENDOR=signature`), переключение на ClamAV выполняется через переменные окружения.
- История изменений задач и отдельные поля номера и даты создания.
- Шрифты загружаются локально, внешние URL исключены из SRI.
...
- Dependabot еженедельно обновляет npm-зависимости, отдельный job CI `audit-deps` с `audit-ci` падает при уязвимостях уровня `high`.
...
5. Аудит зависимостей выполняется `./scripts/audit_deps.sh`; `pre_pr_check.sh` вызывает его автоматически
6. Слабые уязвимости пропускаются флагом `--audit-level high`, обход ложных срабатываний описан в `docs/security/audit_ci_false_positives.md`
...
21. В коллекции `roles` хранится список прав; страница `/cp/settings` позволяет их менять
...
32. Значения enum задаются в `packages/shared/src/taskFields.ts`, страница справочников удалена
...
53. Мини-приложение открывается в браузере без флага `?browser=1`
...
71. Списки значений загружаются из `packages/shared/src/taskFields.ts` без вызова API
...
106. Функции работы с Google Maps вынесены в общий модуль `packages/shared/src/mapUtils.ts`
...
239. Веб‑клиент импортирует mapUtils из пакета `shared`
...
265. Текстовые ответы бота вынесены в `apps/api/src/messages.ts`
...
271. Исключение `apps/api/src/api/*.js` удалено из tsconfig, план миграции обновлён
