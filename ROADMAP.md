...
16. В коллекции `roles` хранится список прав; страница `/cp/roles` позволяет их менять
16. Страница `/cp/admin` содержит только раздел логов
17. Пагинация списка задач для уменьшения нагрузки на клиент
18. Автоматическое формирование `slug` для человекочитаемого URL
19. Улучшение SEO и доступности (robots.txt, aria‑label, meta description)
18. Расширенная модель задачи с логистикой, закупками и работами
19. Поддержка enum-полей для типов транспорта, оплаты и статусов
20. CRUD-эндпойнты `/api/v1/tasks` с полной формой заявки
21. Учёт этих полей в веб-клиенте и контроллерах
22. Значения enum задаются в `taskFields.cjs`, страница справочников удалена
...
115. `authFetch` перенаправляет на `/login` при отсутствии токена или статусе 401/403
116. Срок действия JWT увеличен до семи дней
117. Подтверждение кода админа обновляет роль существующего пользователя на `admin`
...
108. Маршрут `/cp/admin` объявлен как `/*splat` для Express 5
...
113. В задачах имена исполнителей и контролёров отображаются ссылками на профиль
115. `authFetch` перенаправляет на `/login` при отсутствии токена или статусе 401/403
116. Запросы `/api/v1/users` и `/api/v1/departments` выполняются только администратором
117. Исправлена ошибка инициализации переменной isAdmin в TasksPage
118. Панель управления доступна по пути `/cp`, кнопка «Админка» расположена в шапке.
119. В ответах API добавлено поле `telegram_username` для отображения имени.
120. Заглушка аватара заменена иконкой `UserCircleIcon`.
121. При вводе админского кода роль пользователя обновляется на `admin`.
122. Исправлены ошибки ESLint в контроллерах routes и tasks
123. Исправлена загрузка списка пользователей в TasksPage для обычных пользователей
124. Загрузка задач в канбане корректно парсит ответ сервера
125. Имена исполнителей и контролёров в форме отображаются после подгрузки пользователей
126. Добавлены тесты админских маршрутов
127. Исправлено обновление роли на страницах задач при смене токена
